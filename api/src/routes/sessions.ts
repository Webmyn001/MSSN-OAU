...
} from '../services/academic-session'
import { createSessionSchema, listSessionsQuerySchema } from '../schemas/sessions'
...
sessionsRoute.post('/', authMiddleware, zValidator('json', createSessionSchema), async c => {
	try {
		await requireExco(c)
