...
import { getActiveSession, getSessionById } from '../services/academic-session'
import {
	createExco,
	getExcosForSession,
	getExcoById,
...
import { inviteExcoSchema, listExcosQuerySchema, updateExcoSchema } from '../schemas/excos'
import { ConflictError } from '../lib/errors'

const excosRoute = new Hono()

// * POST /excos/invite - Invite Exco member
excosRoute.post('/invite', authMiddleware, zValidator('json', inviteExcoSchema), async c => {
	try {
		await requireExco(c)
		const { email, fullName: _fullName, position, phone: _phone, sessionId } = c.req.valid('json')
		...
		if (existingExco) {
			return errorResponse(c, 'User is already an Exco for this session', 'ALREADY_EXCO', 409)
		}
...
excosRoute.patch('/:excoId', authMiddleware, zValidator('json', updateExcoSchema), async c => {
	try {
		await requireExco(c)
